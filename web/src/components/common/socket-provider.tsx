...
export type MessageStatusEventCallback = (
  payload: MessageStatusPayload
) => void;
export type MessageReactionEventCallback = (
...
export function useSocket(): UseSocketState & UseSocketActions {
  ...
  const scheduleReconnect = useCallback(() => {
    ...
    console.log(
      `🔄 Scheduling reconnect attempt ${currentAttempts + 1} in ${delay}ms`
    );
    updateState({
  ...
  const setupMessageHandlers = useCallback(
    (socket: Socket) => {
      ...
      socket.on(
        WEBSOCKET_EVENTS.MESSAGE_STATUS,
        (payload: MessageStatusPayload) => {
      ...
      socket.on(
        WEBSOCKET_EVENTS.MESSAGE_REACTION,
        (payload: MessageReactionPayload) => {
      ...
      socket.on(
        WEBSOCKET_EVENTS.FILE_UPLOAD_PROGRESS,
      ...
      socket.on(
        WEBSOCKET_EVENTS.FILE_UPLOAD_COMPLETE,
        (payload: FileUploadPayload) => {
      ...
      socket.on(
        WEBSOCKET_EVENTS.FILE_UPLOAD_ERROR,
        (payload: FileUploadPayload) => {
  ...
  const on = useCallback(
    (event: string, callback: (...args: any[]) => void) => {
      if (state.socket) {
        ...
        console.warn(
          `⚠️ Trying to add listener for ${event} but socket is not connected`
        );
      }
    },
    [state.socket]
  ...
  const setOnlineStatus = useCallback(
    (isOnline: boolean) => {
      emit(
        isOnline ? WEBSOCKET_EVENTS.USER_ONLINE : WEBSOCKET_EVENTS.USER_OFFLINE,
        {}
      );
    },
    [emit]
  ...
  const sendFileUploadComplete = useCallback(
    (uploadId: string, fileUrl: string) => {
      emit(WEBSOCKET_EVENTS.FILE_UPLOAD_COMPLETE, { uploadId, fileUrl });
    },
    [emit]
  );

  const sendFileUploadError = useCallback(
...
export function useConnectionStatus() {
  const { connectionState, isConnected, reconnectAttempts, latency } =
    useSocket();
  ...
  const statusLabel = useMemo(() => {
    switch (connectionState) {
      ...
      case "connected":
        return "Connected";
      case "reconnecting":
        return `Reconnecting... (${reconnectAttempts}/${WEBSOCKET_CONFIG.MAX_RECONNECT_ATTEMPTS})`;
      case "failed":
